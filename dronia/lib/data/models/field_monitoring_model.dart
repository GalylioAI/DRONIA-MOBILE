...
/// Vegetation index type (Copernicus Sentinel-2)
...
/// Soil Moisture data point (Sentinel-1 radar)
class SoilMoistureDataPoint {
  final DateTime date;
  final double? moisture; // 0-1 scale
  final double? vv; // VV polarization
  final double? vh; // VH polarization
  final String source;
  ...
  SoilMoistureDataPoint({
    ...
    this.vv,
    this.vh,
    this.source = 'Sentinel-1',
  });
}

/// Air Quality data point (Sentinel-5P)
class AirQualityDataPoint {
  final DateTime date;
  final double? no2; // NO₂ in mol/m²
  final double? so2; // SO₂ in mol/m²
  final double? co; // CO in mol/m²
  final double? o3; // O₃ in mol/m²
  final double? aerosol; // Aerosol index
  final String source;
  ...
  AirQualityDataPoint({
    ...
    this.o3,
    this.aerosol,
    this.source = 'Sentinel-5P',
  });

  /// Get Air Quality Index (simplified)
  String get aqiLevel {
    if (no2 == null) return 'N/A';
    // NO2 thresholds in mol/m² (simplified)
    if (no2! < 0.00005) return 'Good';
    if (no2! < 0.0001) return 'Moderate';
    if (no2! < 0.0002) return 'Unhealthy';
    return 'Hazardous';
  }
}
